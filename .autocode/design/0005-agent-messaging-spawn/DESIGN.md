...
The header is a single HTML comment (survives rendering, parseable by one grep) carrying `id` (message id), `from`, `to` (omitted for broadcast), `type` (directed|broadcast), `context` (thread id), optional `task` (delegated-task id), `ts` (ISO8601, needed locally for ordering). The body below it is the message text.
...
6. Borrow A2A field names loosely; the addressing model is our own. `id`/`context`/`task` mirror A2A's `messageId`/`contextId`/`taskId`. Everything else is local: A2A (v1.0.1 at time of writing) has no `from`/`to` on Message (direction is a two-value `role` enum), no `type` field, no broadcast (messaging is strictly bilateral), and no Message timestamp; its AgentCard requires `skills`, an endpoint, and `capabilities` in every spec version, so our card (name, description, pane, server, task) is a local name tag, not an AgentCard subset. This channel is not the A2A protocol and does not aim at interop; the `a2a-` skill prefix reads as generic "agent-to-agent". Rejected: a real A2A HTTP/JSON-RPC server (overkill for local panes); tracking the evolving spec shape for a local file channel with no interop consumer.
...
9. Receiving is opt-in; sending auto-registers. A session receives messages only after its agent dir exists, via explicit `/a2a-register` or by being spawned; that keeps the per-prompt hooks a fast no-op by default. Outbound skills (`a2a-send`, `a2a-broadcast`, `a2a-spawn`) auto-register the sender under the default name when unregistered, since they need a `from` identity anyway. The default-name scheme (`<session_name>-<window_index>`) is defined once, in channel-core.
...
| [messaging-skills](units/messaging-skills.md) | `/a2a-send`, `/a2a-broadcast` skills + shims (write message files, stamp header fields, auto-register sender) | channel-core |
...
| [spawn-skill](units/spawn-skill.md) | `/a2a-spawn` skill: tmux window/session, launch `claude`, seed via handover file, register | channel-core |
...
- Stored `tmux:` coords went stale on window moves; `repo:` had no reader; `a2a-agents` printed a `state` field decision 6 had dropped; the Units table still said "best-effort poke". Resolved: coords derived at read time, `repo`/`state` dropped, table fixed.
