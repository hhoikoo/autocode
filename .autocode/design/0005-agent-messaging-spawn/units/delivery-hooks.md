...
`plugins/autocode/hooks/comms-inject.sh`
...
`plugins/autocode/hooks/comms-rewake.sh`
- The `Stop` hook entry carries `asyncRewake: true` AND an explicit `timeout` (verified: asyncRewake defaults to 60 seconds and allows up to 86400; without an explicit value the "bounded poll" silently becomes a one-minute idle window). Set `timeout: 28800` (8h; covers a session left idle across a working day, so a peer's message still wakes it instead of queuing until the next prompt). The poll window equals the hook timeout, stated in a `leanness:` comment (the value is a one-number change; max 86400).
...
`plugins/autocode/hooks/comms-cleanup.sh`
...
- This is the automatic counterpart of `/comms-deregister` and closes two holes lazy reaping cannot: an orphaned poller when the user quits without deregistering, and a pane whose shell outlives `claude` (pane liveness alone would keep that agent "live" forever).

### Files to modify

`plugins/autocode/hooks/hooks.json`
- Add a `UserPromptSubmit` event with one hook -> `${CLAUDE_PLUGIN_ROOT}/hooks/comms-inject.sh`.
- Add a second `SessionStart` matcher/hook entry -> `${CLAUDE_PLUGIN_ROOT}/hooks/comms-inject.sh`, without disturbing the existing `startup`/check-install entry (multiple hooks may bind one event).
- Add a second `Stop` hook entry -> `${CLAUDE_PLUGIN_ROOT}/hooks/comms-rewake.sh` with `asyncRewake: true` and `timeout: 28800` (seconds; the hook `timeout` unit is seconds), alongside the existing check-progress-log entry.
- Add a `SessionEnd` event -> `${CLAUDE_PLUGIN_ROOT}/hooks/comms-cleanup.sh`.
- All commands use `${CLAUDE_PLUGIN_ROOT}` per the existing convention (hooks.json:1-25).

```
hooks.json
  UserPromptSubmit -> comms-inject.sh                        (new)
  SessionStart     -> check-install.sh (startup)           (existing)
                   -> comms-inject.sh                         (new)
  Stop             -> check-progress-log.sh                 (existing)
                   -> comms-rewake.sh (asyncRewake, timeout)  (new)
  SessionEnd       -> comms-cleanup.sh                        (new)
```
...
One runnable bash self-check (assert-based, no framework): `plugins/autocode/hooks/comms-inject.test.sh`.

- Build a temp messages dir with one agent whose card `pane:` matches a stubbed `$TMUX_PANE`, seed two message files in `new/`, pipe crafted `UserPromptSubmit` hook JSON into `comms-inject.sh`, and assert: stdout is valid JSON whose `hookSpecificOutput.additionalContext` contains both messages, `new/` is empty, and both files are in `cur/`.
