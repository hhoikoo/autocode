...
- Message file `new/<ts>-<uuid>.md`: one message per file, filename orders delivery. A single HTML-comment header `<!-- comms-msg id=... from=... to=... type=... ts=... -->` (`to` omitted for broadcast), then the body text. No `context`/`task` fields: no v1 reader (DESIGN.md decision 6).
...
| `send <agent> <text>` (and the broadcast fan-out) | compose the header (`id` from `uuidgen`, `from` self, `ts` ISO8601, `type`), write to a temp file in the messages dir (same filesystem so `mv` is an atomic rename), `mv` into `<agent>/new/<ts>-<uuid>.md`. No locks: concurrent senders write distinct files | `DESIGN.md` decisions 3-4 |
