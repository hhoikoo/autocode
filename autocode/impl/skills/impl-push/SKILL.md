...
4. Delegate to `git-commit` (stages the code plus `PROGRESS.md`, `progress/<slug>.md`, and the recap artifacts under `.autocode/design/<design_id>-<shortname>/recap/<slug>/`: `RECAP.md` and any SVG assets). Verification is owned by the repo's pre-commit hooks; this skill does not run a verify step.
