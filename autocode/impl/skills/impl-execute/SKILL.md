...
5. Commit at meaningful checkpoints. Delegate each commit to `git-commit` (a logical unit per commit, not one monolith, not per-file noise). Committing drives progress logging via the Stop hook + `progress-logger`; do not log progress here and do not append `PROGRESS.md` (that is `impl-push`). Under `--no-commit`, leave changes uncommitted (see Args). With `--auto`, report the list of files written in the result block instead of commits.
...
- Delegate commits to `git-commit`; never inline commit logic (under `--no-commit`, leave changes uncommitted, see Args; the workflow's Commit step owns commits and progress logging).
