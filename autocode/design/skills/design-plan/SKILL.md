...
3. Gap identification. List every assumption, every unknown library/API, every "I'm guessing X". Decide per gap:
   ...
   Run the leanness ladder over the whole shape before decomposing: does each proposed unit or component need to exist (YAGNI), does stdlib / a native feature / an existing dep cover it, is there a simpler decomposition. Cut speculative scope here, where it is cheapest to delete. Never cut the `Lazy is not careless` carve-outs from the active style.
...
5. Write the folder.
   ...
   - Without `--temp` only: write `<folder>/PROGRESS.md` containing exactly one line: `# Progress: <shortname>` (the canonical header; format authority: the `## PROGRESS.md` section of `design-folder.md`). This is the common-ancestor seed; it rides the design PR to `main`, becoming the merge-base for every unit branch before any unit appends to it. For flat single-unit designs write it unconditionally (inert when only one unit runs; no concurrent appending). Do not write `PROGRESS.md` in the `--temp` path (temp designs are throwaway and must not touch the repo merge-base).
