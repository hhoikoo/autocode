...
1. Run `bash ~/.autocode/autocode/git/skills/git-commit/scripts/commit.sh gather`. The `gather` subcommand prints a single JSON object to stdout with keys: `staged`, `unstaged`, `untracked`, `log_recent`, `current_branch`, `upstream`, `ahead`, `behind`.
...
5. Run `bash ~/.autocode/autocode/git/skills/git-commit/scripts/commit.sh commit "$msg"`. The script runs `git commit -F "$msg"`.
...
7. If the branch tracks a remote and `ahead > 0`, run `bash ~/.autocode/autocode/git/skills/git-commit/scripts/commit.sh push`. If `upstream` is empty, tell the user the next step is opening a PR.
